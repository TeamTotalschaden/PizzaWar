...
public class EnemyWave {
		...
		if (gameWorld.getGameTime() > nextSpawnTime) {
			...
			if (currentWave < subGroups.size()) {
				if (currentWave + 1 < subGroups.size()) {
					nextSpawnTime = gameWorld.getGameTime()
	...
	private void spawnEnemies(EnemyGroup currentGroup) {
		...
		Vector2f groupCenterPosition = gameWorld.pizzaPositionFromRad(
				randomRad, 0.85f);
		for (Object[] spawnData : currentGroup.enemyTypeInfo) {
			for (int a = 0; a < (Integer) spawnData[0]; ++a) {
				gameWorld.addEntity(new Alien(gameWorld, groupCenterPosition.x,
						groupCenterPosition.y, (AlienType) spawnData[1],
						(Integer) spawnData[2]));
			}
		}
	}
