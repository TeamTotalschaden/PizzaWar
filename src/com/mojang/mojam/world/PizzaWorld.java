...
public class PizzaWorld {
    ...
    private static String[] scoreFiller = {"000000000", "00000000", "0000000", "000000", "00000", "0000", "000", "00", "0" , ""};
	...
	if (upgradeMenu.isVisible()) {
	    ...
	    String scoremsg = "Score: " + scoreFiller[score.length()>9?9:score.length()] + score;
	    g.drawString(scoremsg,
		    container.getWidth() - g.getFont().getWidth(scoremsg) - 10, 10);
	    
	    String cash = "" + player.getCash();
	    String cashmsg = "Money: " + cashFiller[cash.length()>9?9:cash.length()] + cash;
	    g.drawString(cashmsg,
		    container.getWidth() - g.getFont().getWidth(cashmsg) - 10, g.getFont().getLineHeight() + 10);
	}
    }
