...
public class Alien extends HealthEntity {
	...
	private static final String[] animations = new String[] {
			"actors/alien1.png", "actors/alien2.png", "actors/alien3.png",
			"actors/alien4.png", "actors/alien5.png", "actors/alien6.png" };
	...
	public Alien(PizzaWorld world, float x, float z, AlienType type,
		...
		this.entityType = EntityType.EnemyEntity;
		try {
			int size = 32;
			...
			alienAnim.setCurrentFrame(random.nextInt(alienSprite
					.getHorizontalCount()));
	...
	public boolean update(GameContainer slickContainer, int deltaMS) {
		...
		if (!super.update(slickContainer, deltaMS)) {
			...
			world.getPlayer().addScore(
					(int) (score * DifficultyState.diffMulti));
			...
			world.getCamera().addScreenShake(
					type == AlienType.BIGGUS ? 2.0f : 1.0f);
			return false;
		}
	...
	public void renderGroundLayer(GameContainer slickContainer, Graphics g,
		...
		if (type == AlienType.EYE_RED || type == AlienType.EYE_BLUE) {
			...
			super.renderGroundLayerWithMult(slickContainer, g, camera,
					shadowMult);
		} else {
	...
	public float getMaxHealth() {
		switch (type == null ? AlienType.NULL : type) {
		case ATTACK_NUKE:
		...
		case SHOOTER:
		default:
			return 100.0f + 25.0f * challengeLevel;
		}
	}
