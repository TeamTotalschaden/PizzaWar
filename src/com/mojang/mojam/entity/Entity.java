...
public abstract class Entity {
	public enum EntityType {
		GenericEntity, PlayerEntity, EnemyEntity, ProjectileEntity, BuildingEntity,
	}
	...
	public abstract void render(GameContainer slickContainer, Graphics g,
			Camera camera);
	...
	public void renderGroundLayerWithMult(GameContainer slickContainer,
			Graphics g, Camera camera, Color shadowMult) {
		if (y < 0) {
			return;
		}
		...
		g.drawImage(shadowImage, x - camera.getX() - halfShadowX,
				z - camera.getY() - halfShadowY, x - camera.getX()
						+ halfShadowX, z - camera.getY() + halfShadowY, 0, 0,
				32, 32, shadowMult);
	}
