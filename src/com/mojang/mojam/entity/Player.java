...
public class Player extends HealthEntity {
	...
	private float beamTimeRechargeAmount = 5000;
	...
	private static final float[][] shootOffsetZ = new float[][] {
			{ -2, -5, 5, -3, 2, -5, -5, 4 }, { -5, -1, 0, 3, -5, 1, -5, 4 } };

	private int resources[] = new int[PizzaResource.NUM_BUY_RESOURCES];
	private int score;
	private int cash;
	...
	public boolean update(GameContainer slickContainer, int deltaMS) {
		...
		if (input.isKeyDown(Input.KEY_SPACE)) {
			...
			engineStrength += (0 - engineStrength) * engineFuelDecrease
					* deltaMS * .001f;
		...
		updateFriction(isMoving);
		{
			acceleration.x = -(velocity.x * playerFriction) + direction.x
					* getPlayerSpeed();
			acceleration.z = -(velocity.z * playerFriction) + direction.y
					* getPlayerSpeed();
		}
		acceleration.y = -9.8f;
		...
		if (y < 0) {
			if (world.isOnPizza(x, z)) {
				if (velocity.y < -13) {
					world.addParticle(new ShockwaveParticle(
							world,
							x,
							z,
							2.0f + attributes.levels[Attributes.SHOCKWAVE] * 1.0f));
					...
					for (Entity e : world.getEntitiesInRange(x, z, range)) {
						if (e instanceof Alien || e instanceof PizzaBubble) {
							...
							{
								float scale = (1.0f - (float) Math
										.sqrt(distSqr) / range);
								float force = 400.0f * scale;
								Vector3f push = (Vector3f) new Vector3f(dx, 0,
										dz).normalise();
								push.x *= force;
								push.z *= force;
								e.push(push);
								((HealthEntity) e)
										.hurt((20 + attributes.levels[Attributes.SHOCKWAVE] * 20)
												* scale);
							}
						}
					}
		...
		if (input.isMouseButtonDown(Input.MOUSE_LEFT_BUTTON)) {
			if (currentLifeTimeMS > lastShotFiredTimeMS + getFireDelay()) {
				...
				if (attributes.levels[Attributes.FIRERATE] == 5) {
					...
					double angle = Math.atan2(playerDirection.y,
							playerDirection.x);
					Vector3f shootDir = new Vector3f((float) Math.cos(angle
							- Math.PI * .05), 0, (float) Math.sin(angle
							- Math.PI * .05));
					shootBullet(aimX, aimZ, 0, shootDir);
					shootDir = new Vector3f((float) Math.cos(angle + Math.PI
							* .05), 0, (float) Math.sin(angle + Math.PI * .05));
				...
				if (attributes.levels[Attributes.FIRE_DAMAGE_LEVEL] <= 1) {
					Sounds.getInstance().playSound(Sounds.SHOT, x, y, z,
							.95f + random.nextFloat() * .1f, 1.0f);
				} else {
					Sounds.getInstance().playSound(Sounds.SHOT2, x, y, z,
							.95f + random.nextFloat() * .1f, 1.0f);
				}
			}
		}
		...
		if (nextEngineParticleMS <= 0) {
			...
			world.addParticle(new AnimationParticle(world, x, y, z,
					AnimationParticle.engineAnimation));
		}
		...
		if (y < -100) {
			...
			health += deltaMS * .005f
					* (1.0f + attributes.levels[Attributes.REGENERATE] * .5f);
		}
	...
	private void shootBullet(float tx, float tz, int shootArm) {
		Vector3f projectileDirection = new Vector3f(playerDirection.x, 0,
				playerDirection.y);
		shootBullet(tx, tz, shootArm, projectileDirection);
	}

	private void shootBullet(float tx, float tz, int shootArm,
			Vector3f projectileDirection) {

		double targetDistance = Math.sqrt((x - tx) * (x - tx) + (z - tz)
				* (z - tz));
		...
		Projectile bullet = new Projectile(world,
				attributes.levels[Attributes.FIRE_DAMAGE_LEVEL], x
						+ shootOffsetX[shootArm][currentAnimation],
				shootHeight, z + shootOffsetZ[shootArm][currentAnimation],
		...
		if (activatedBeam) {
			...
			if (beamTime <= 0) {
				...
				if (beamShootTimeMS <= 0) {
					beamShootTimeMS = 180;
					world.addParticle(new AnimationParticle(world, aimX, 0,
							aimZ, AnimationParticle.explodeAnimation));
			...
			beamTime += deltaMS * beamTimeRechargeAmount
					* beamTimeRechargeAmount;
		}
	}
	...
	public void render(GameContainer slickContainer, Graphics g, Camera camera) {
		...
		if (playerDirection.y >= 0) {
			a.draw(x - camera.getX() - a.getWidth() / 2,
					z - camera.getY() - a.getHeight() - y, playerColor);
			super.render(slickContainer, g, camera);
		}

		if (beamTime > 0 && activatedBeam) {
			double angle = playerDirection.getTheta() * Math.PI / 180.0
					+ Math.PI * .5;
			float armY = y + 10;
			float armX = x + shootOffsetX[0][currentAnimation] - camera.getX();
			float armZ = z + shootOffsetZ[0][currentAnimation] - camera.getY()
					- armY;
			renderLaserBeam(g, angle, armX, armZ, aimX - camera.getX(), aimZ
					- camera.getY());
	...
	private void renderLaserBeam(Graphics g, double angle, float sx, float sy,
			float ex, float ey) {
		...
		for (int b = 0; b < 3; b++) {
			...
			g.setColor(new Color(.5f + .25f * b, .33f * b * animScale, .2f * b
					* animScale, .75f));
			/* @formatter:off */
	...
	public boolean collidesWith(Entity other) {
		if ((other instanceof Alien) || (other instanceof Pickup)
				|| (other instanceof AlienProjectile)) {
			return ((y < other.y + other.getEntityHeight()) && (y + this
					.getEntityHeight()) > (other.y));
		} else if (y < 32 && other instanceof Artichoke) {
			return true;
		} else if (y < 8 && other instanceof PizzaBubble) {
			return true;
		}
		return false;
	}
	...
	protected void onCollide(Entity entity) {
		if (entity instanceof Artichoke || entity instanceof PizzaBubble) {
	...
	public static class Attributes {
		...
		public static final String[] ICONS = { "GUI/shopicons_health.png",
				...
				"GUI/shopicons_nova.png", "GUI/shopicons_regenerate.png",
	...
	public void addBeamTime(float time) {
		if (time + beamTime > getMaxBeamTime()) {
			beamTime = getMaxBeamTime();
		} else {
			beamTime += time;
		}
	...
	public void setBeamTime(float time) {
		beamTime = time;
	}
