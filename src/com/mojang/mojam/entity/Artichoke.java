...
public class Artichoke extends HealthEntity {
	...
	public Artichoke(PizzaWorld world, float x, float z) {
		...
		try {
			anim = new Animation(new SpriteSheet("actors/artichoke.png", 128,
					128), 200);
			wall1 = new Animation(
					new SpriteSheet("actors/wall1.png", 192, 192), 200);
			wall2 = new Animation(
					new SpriteSheet("actors/wall2.png", 192, 192), 200);
			wall3 = new Animation(
					new SpriteSheet("actors/wall3.png", 192, 192), 200);
	...
	public boolean update(GameContainer slickContainer, int deltaMS) {
		if (lastBlasterFireTime + getTowerBlasterFireRate() < world
				.getGameTime()) {
			lastBlasterFireTime = world.getGameTime();
			if (attributes.levels[ArtichokeAttributes.TOWERBLASTER] > 0) {
				Alien closestEnemy = world.getClosestEnemy(this,
						getTowerBlasterRange());
				if (closestEnemy != null) {
					Vector3f direction = new Vector3f(closestEnemy.x - (x - 2),
							closestEnemy.y - (y + 80), closestEnemy.z - z);
					direction.normalise();
					int projectileLevel = Math
							.max(0,
									attributes.levels[ArtichokeAttributes.TOWERBLASTER] - 3);
					Projectile p = new Projectile(world, projectileLevel,
							x - 2, y + 80, z, direction, 400);
					world.addEntity(p);
			...
			if (attributes.levels[ArtichokeAttributes.SHOCKWAVE] > 0) {
				...
				if (closestEnemy != null) {
					for (int a = 0; a < 36; ++a) {
						...
						Vector3f direction3d = new Vector3f(direction.x, 0.3f,
								direction.y);
						direction3d.normalise();
						NovaProjectile nProjectile = new NovaProjectile(world,
								x + direction.x * getCollisionRadius() / 2, y,
								z + direction.y * getCollisionRadius() / 2,
								direction3d, 400, 150, getNovaDamage());
		...
		if (lastSpiderTime + 4000 < world.getGameTime()) {
			...
			if (attributes.levels[ArtichokeAttributes.SPIDERS] > 0) {
				for (int i = 0; i < attributes.levels[ArtichokeAttributes.SPIDERS]; i++) {
					...
					world.addEntity(new SpiderMine(world, x
							+ (float) Math.cos(angle) * range, z
							+ (float) Math.sin(angle) * range * .5f));
				}
	...
	public static class ArtichokeAttributes {
		...
		public static final String[] DESCS = { "Fortification",
				"Tower Blaster", "Nova Defense", "Defense Spiders" };
		public static final String[] ICONS = { "GUI/shopicons_base_walls.png",
				"GUI/shopicons_base_shot.png", "GUI/shopicons_base_blast.png",
				"GUI/shopicons_base_blast.png" };
		...
		public int[] getUpgradeCost(int i) {
			if (i == FORTIFICATION) {
				if (owner.getFortificationLevel() < owner
						.getMaxFortificationLevel()) {
