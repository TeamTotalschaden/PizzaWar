...
public class ShockwaveParticle extends Particle {
	...
	public ShockwaveParticle(PizzaWorld world, float x, float z, float maxSize) {
		...
		try {
			activeImage = new Image("particles/shockwave.png");
	...
	public void renderGroundLayer(GameContainer slickContainer, Graphics g,
		...
		fadeColor.a = alpha;
		g.drawImage(activeImage, cx, cy, cx + w, cy + h, 0, 0,
				activeImage.getWidth(), activeImage.getHeight(), fadeColor);
	...
	public static class AlienWarning extends ShockwaveParticle {
		...
		public AlienWarning(PizzaWorld world, float x, float z) {
			...
			try {
				activeImage = new Image("particles/alien_shockwave.png");
			} catch (SlickException e) {
				e.printStackTrace();
			}
		}
