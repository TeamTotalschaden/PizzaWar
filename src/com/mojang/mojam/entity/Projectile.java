...
public class Projectile extends Entity {
	...
	protected Projectile(PizzaWorld world, float x, float y, float z,
	...
	public Projectile(PizzaWorld world, int projectileLevel, float x, float y,
			float z, Vector3f direction, float speed) {
		this(world, x, y, z, direction, speed);
		try {
			if (projectileLevel == 0) {
				bulletImage = new OffsetImage("projectiles/bullet.png");
				blastRadius = 0;
				damage = 20;
			} else if (projectileLevel == 1) {
				bulletImage = new OffsetImage("projectiles/bullet1.png");
				blastRadius = 30.0f;
				damage = 35;
			} else if (projectileLevel == 2) {
				bulletImage = new OffsetImage("projectiles/bullet2.png");
				blastRadius = 50.0f;
				damage = 45;
			} else {
				bulletImage = new OffsetImage("projectiles/bullet3.png");
				blastRadius = 50.0f;
				damage = 50;
			}
			bulletOffset = new Vector2f(bulletImage.getWidth() / 2,
	...
	public boolean update(GameContainer slickContainer, int deltaMS) {
		...
		if (!isOutOfWorld() && !isRemoved()
				&& (y > 0 || !world.isOnPizza(x, z))) {
			return true;
		} else {
			onDeath();
			return false;
	...
	protected void onDeath() {
		...
		world.addParticle(new AnimationParticle(world, x, bulletY, z,
				AnimationParticle.bulletImpactAnimation));
		Sounds.getInstance().playSound(Sounds.SHOT_LAND, x, y, z, 1.0f, .5f);
	}

	protected void renderSelf(GameContainer slickContainer, Graphics g,
			Camera camera, float yOff) {
		double angleDegrees = Math.atan2(direction.z, direction.x) * 180.0
				/ Math.PI;
		g.pushTransform();
		g.rotate(x - camera.getX(), z - camera.getY() - yOff,
				(float) angleDegrees + 90);
		g.drawImage(bulletImage, x - camera.getX() - bulletOffset.x,
				z - camera.getY() - bulletOffset.y - yOff);
		g.popTransform();
	...
	public void onCollide(Entity entity) {
		if (!isRemoved()) {
			if (entity.entityType == EntityType.EnemyEntity
				...
				entity.push(new Vector3f(direction.x * pushAmount, 0,
						direction.z * pushAmount));
				setRemoved();
