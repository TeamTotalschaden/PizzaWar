...
public class UpgradeMenu {
	...
	private final static int[] UPGRADE_ORDER = { Attributes.FIRE_DAMAGE_LEVEL,
			Attributes.FIRERATE, Attributes.SPEED, Attributes.REGENERATE,
			Attributes.HEALTH, Attributes.BEAM_DURATION, Attributes.SHOCKWAVE,
			Attributes.JETPACK, };
	private final static int[] ARTICHOKE_UPGRADE_ORDER = {
			Artichoke.ArtichokeAttributes.FORTIFICATION,
			Artichoke.ArtichokeAttributes.TOWERBLASTER,
			Artichoke.ArtichokeAttributes.SHOCKWAVE,
			Artichoke.ArtichokeAttributes.SPIDERS };
	...
	public UpgradeMenu(Player player, Artichoke artichoke) {
		...
		try {
			for (int i = 0; i < upgradeIcons.length; i++) {
				upgradeIcons[i] = new Animation(new SpriteSheet(
						Player.Attributes.ICONS[i], 48, 48), 200);
			}
			for (int i = 0; i < artichokeUpgradeIcons.length; i++) {
				artichokeUpgradeIcons[i] = new Animation(new SpriteSheet(
						Artichoke.ArtichokeAttributes.ICONS[i], 48, 48), 200);
			}
		...
		if (input.isMousePressed(0)) {
			...
			if (mouseX >= xPos && mouseX <= xPos + 48) {
				...
				if (selectedUpgrade >= 0
						&& selectedUpgrade < UPGRADE_ORDER.length) {
			...
			if (mouseX >= xPos && mouseX <= xPos + 48) {
				int selectedUpgrade = (mouseY - yPos) / UPGRADE_SPACING;
				if (selectedUpgrade >= 0
						&& selectedUpgrade < ARTICHOKE_UPGRADE_ORDER.length) {
	...
	public void render(GameContainer container, Graphics g) {
		...
		Gui.renderResourceList(g, player.getResources(), xPos, yPos,
				"Wealth: ", 400);
		yPos += 50;

		g.setColor(shade);
		g.fillRect(xPos - 8, yPos - 8, 312, Attributes.COUNT * UPGRADE_SPACING
				+ 12);
		...
		for (int i = 0; i < Attributes.COUNT; i++) {
			...
			g.setColor(Color.white);
			Gui.renderResourceList(g, player.getUpgradeCost(upgrade),
					xPos + 70, yPos + 4, "", 280 - 70);
			if (attributes.levels[upgrade] >= attributes.getMaxLevel(upgrade)) {
				g.setColor(goldColor);
				g.drawString(Attributes.DESCS[upgrade] + ": MAX", xPos + 54,
						yPos + 31);
			} else {
				g.drawString(
						Attributes.DESCS[upgrade] + ": "
								+ attributes.levels[upgrade] + " / "
								+ attributes.getMaxLevel(upgrade), xPos + 54,
						yPos + 31);
			}
		...
		g.fillRect(xPos - 8, yPos - 8, 312, Attributes.COUNT * UPGRADE_SPACING
				+ 12);
		Artichoke.ArtichokeAttributes artichokeAttributes = artichoke
				.getAttributes();
		for (int i = 0; i < ARTICHOKE_UPGRADE_ORDER.length; i++) {
			...
			if (canAfford(artichoke.getUpgradeCost(upgrade))) {
				...
				artichokeUpgradeIcons[upgrade]
						.draw(xPos, yPos, cantAffordColor);
			}

			g.setColor(Color.white);
			Gui.renderResourceList(g,
					artichokeAttributes.getUpgradeCost(upgrade), xPos + 70,
					yPos + 4, "", 280 - 70);
			if (artichokeAttributes.levels[upgrade] >= artichokeAttributes
					.getMaxLevel(upgrade)) {
				g.setColor(goldColor);
				g.drawString(Artichoke.ArtichokeAttributes.DESCS[upgrade]
						+ ": MAX", xPos + 54, yPos + 31);
			} else {
				g.drawString(Artichoke.ArtichokeAttributes.DESCS[upgrade]
						+ ": " + artichokeAttributes.levels[upgrade] + " / "
						+ artichokeAttributes.getMaxLevel(upgrade), xPos + 54,
						yPos + 31);
			}
			yPos += UPGRADE_SPACING;
		}
